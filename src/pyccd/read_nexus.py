    ...
    with open(file, 'r', encoding="utf-8") as f:
        for line in f:
            if re_tree.match(line):
                ...
                ete4tree = ete4.Tree(new_tree_string, format=1)

                if breath_trees:
                    # adjusting the tree to contain the blockcount label and correct node names
                    _breath_label_nodes(ete4tree)

                trees.append(ete4tree)
    # if only label_transm_history is set to true this won't make sense anyway
    if breath_trees and label_transm_history:
        for ete4tree in trees:
            label_transmission_tree(ete4tree)
    return trees


def _breath_label_nodes(ete4tree):
    """
    Annotating the node names and blockcount values to an ete4.Tree.
    ...
    :param ete4tree: ete4.Tree that will get blockcount annotations from node names
    :type ete4tree: ete4.Tree
    """
    for node in ete4tree.traverse("levelorder"):
